...
- Exposure-load shadow collection packet workflow sprints are available through `risk-engine --exposure-load-shadow-collection-packet-workflow-sprint --exposure-load-shadow-collection <exposure_load_shadow_collection_template.csv> --output-dir outputs --experiment-id <id>`. They write `exposure_load_shadow_collection_packet_manifest.csv`, `exposure_load_shadow_collection_packet_checklist.csv`, `exposure_load_shadow_collection_packet_audit_trail.csv`, `exposure_load_shadow_collection_reviewer_instructions.md`, `exposure_load_shadow_collection_packet_workflow.json`, `exposure_load_shadow_collection_packet_workflow_report.md`, and one de-identified markdown file per packet under `review_packets/`; the sprint prepares reviewer packet operations and audit seeds without filling evidence or changing calibration/product readiness.
- Exposure-load shadow collection evidence prefill sprints are available through `risk-engine --exposure-load-shadow-collection-evidence-prefill-sprint --exposure-load-shadow-review-packets <exposure_load_shadow_review_packets.csv> --output-dir outputs --experiment-id <id>`. They write `exposure_load_shadow_collection_prefilled.csv`, `exposure_load_shadow_collection_prefill_validation.csv`, `exposure_load_shadow_collection_prefill_excluded.csv`, `exposure_load_shadow_collection_evidence_prefill.json`, and `exposure_load_shadow_collection_evidence_prefill_report.md`; the sprint fills retained-channel collection rows with replay-derived season, source-eligibility, episode, observed-event, and captured-event evidence while leaving reviewer judgment fields blank.
- Exposure-load shadow collection summary sprints are available through `risk-engine --exposure-load-shadow-collection-summary-sprint --exposure-load-shadow-collection <exposure_load_shadow_collection_template.csv> --output-dir outputs --experiment-id <id>`. They write `exposure_load_shadow_collection_validation.csv`, `exposure_load_shadow_collection_channel_summary.csv`, `exposure_load_shadow_collection_summary.json`, and `exposure_load_shadow_collection_summary_report.md`; the sprint validates prospective retained-channel collection rows, summarizes source-eligible useful/actionable evidence by channel, and now distinguishes `complete_practitioner_adjudication` rows from `complete_csv_evidence_review` rows while keeping calibration, pilot, dashboard, probability-facing, and autonomous-intervention readiness blocked while required collection rows remain pending or invalid.
- Exposure-load shadow calibration-readiness sprints are available through `risk-engine --exposure-load-shadow-calibration-readiness-sprint --exposure-load-shadow-collection-summary <exposure_load_shadow_collection_summary.json> --output-dir outputs --experiment-id <id>`. They write `exposure_load_shadow_calibration_readiness_channels.csv`, `exposure_load_shadow_calibration_readiness_gaps.csv`, `exposure_load_shadow_calibration_readiness.json`, and `exposure_load_shadow_calibration_readiness_report.md`; the sprint converts completed retained-channel collection evidence into a bounded calibration-readiness decision package, clears only the independent-adjudication research gate when all complete rows are practitioner adjudicated, and still blocks calibration claims, probability-facing output, pilot/dashboard readiness, and autonomous intervention.
- Exposure-load shadow calibration-sensitivity sprints are available through `risk-engine --exposure-load-shadow-calibration-sensitivity-sprint --exposure-load-shadow-calibration-readiness <exposure_load_shadow_calibration_readiness.json> --exposure-load-shadow-collection <practitioner_reviewed_collection.csv> --exposure-load-shadow-event-crosswalk <exposure_load_shadow_event_crosswalk.csv> --output-dir outputs --experiment-id <id>`. They write `exposure_load_shadow_calibration_sensitivity.csv`, `exposure_load_shadow_evidence_dossier.csv`, `exposure_load_shadow_error_modes.csv`, `exposure_load_shadow_calibration_sensitivity.json`, and `exposure_load_shadow_calibration_sensitivity_report.md`; the sprint consolidates practitioner-reviewed retained-channel evidence, captured/missed event context, and bounded sensitivity gates before any calibration research escalation while still blocking calibration claims, probability-facing output, pilot/dashboard readiness, and autonomous intervention.
- Exposure-load shadow error-control sprints are available through `risk-engine --exposure-load-shadow-error-control-sprint --exposure-load-shadow-calibration-sensitivity <exposure_load_shadow_calibration_sensitivity.json> --output-dir outputs --experiment-id <id>`. They write `exposure_load_shadow_error_control_decisions.csv`, `exposure_load_shadow_error_control_evidence_dossier.csv`, `exposure_load_shadow_error_controls.csv`, `exposure_load_shadow_error_control_policy.json`, and `exposure_load_shadow_error_control_report.md`; the sprint defines error-mode controlled calibration research, separates monitoring-useful evidence from prediction/calibration-useful evidence, and preserves probability-facing output, calibration claims, pilot/dashboard readiness, autonomous intervention, and load modification as blocked.
- Exposure-load shadow bounded calibration protocol sprints are available through `risk-engine --exposure-load-shadow-bounded-calibration-protocol-sprint --exposure-load-shadow-error-control-policy <exposure_load_shadow_error_control_policy.json> --output-dir outputs --experiment-id <id>`. They write `exposure_load_shadow_bounded_calibration_protocol_channels.csv`, `exposure_load_shadow_bounded_calibration_evidence_use.csv`, `exposure_load_shadow_bounded_calibration_protocol_gates.csv`, `exposure_load_shadow_bounded_calibration_protocol.json`, and `exposure_load_shadow_bounded_calibration_protocol_report.md`; the sprint authorizes only descriptive shadow calibration stress-test research under miss-fraction, monitoring/prediction-boundary, and packet-review controls while keeping probability-facing output, calibration claims, pilot/dashboard readiness, autonomous intervention, and load modification blocked.
- Exposure-load shadow bounded calibration stress-test sprints are available through `risk-engine --exposure-load-shadow-bounded-calibration-stress-test-sprint --exposure-load-shadow-bounded-calibration-protocol <exposure_load_shadow_bounded_calibration_protocol.json> --output-dir outputs --experiment-id <id>`. They write `exposure_load_shadow_bounded_calibration_stress_channels.csv`, `exposure_load_shadow_bounded_calibration_stress_scenarios.csv`, `exposure_load_shadow_bounded_calibration_stress_gates.csv`, `exposure_load_shadow_bounded_calibration_stress_test.json`, and `exposure_load_shadow_bounded_calibration_stress_test_report.md`; the sprint runs only descriptive stress scenarios and preserves a limited finding when high miss fractions dominate, keeping calibration claims, probability-facing output, pilot/dashboard readiness, autonomous intervention, and load modification blocked.
...
**Interpretation:** The code-side ingest path is ready, but the current blank worksheet cannot advance the project. This is the current user/practitioner involvement boundary: complete the de-identified packet rows first, then re-run ingest and completion validation. Use `docs/prospective_collection_completion_guide.md`, `docs/prospective_collection_completion_process_reference.md`, and `docs/prospective_collection_packet_crosswalk.md` for the exact completion workflow, allowed values, packet map, examples, and validation commands. Do not move to bounded retest, calibration claims, probability-facing output, pilot/dashboard work, autonomous intervention, or load modification until required collection rows are complete and revalidated.
...
**Live results (`exposure_load_shadow_prospective_collection_completion_v1`):**
- Overall recommendation: `continue_prospective_collection_before_bounded_retest`.
- Milestone status: `prospective_collection_incomplete`.
- Bounded retest readiness: `blocked_pending_collection`.
- Production readiness: `not_ready_for_probability_or_pilot`.
- Calibration claim readiness: `not_ready_for_calibration_claims`.
- Pilot/dashboard readiness: `blocked`.
- Load modification readiness: `blocked`.
- Packet rows: 8.
- Complete practitioner packet rows: 0.
...
**Exposure-load shadow prospective evidence gate sprint** - implemented and verified on 2026-05-17.

**What changed:** Added `exposure_load_shadow_prospective_evidence_gate.py`, `run_exposure_load_shadow_prospective_evidence_gate_sprint_experiment(...)`, and the `--exposure-load-shadow-prospective-evidence-gate-sprint` CLI mode. The sprint consumes the bounded stress-test JSON and emits collection targets, packet targets, evidence gates, JSON, and a report.

**Live results (`exposure_load_shadow_prospective_evidence_gate_v1`):**
- Overall recommendation: `collect_prospective_retained_channel_evidence_before_retesting`.
- Milestone status: `prospective_evidence_collection_gate_defined`.
- Production readiness: `not_ready_for_probability_or_pilot`.
- Calibration claim readiness: `not_ready_for_calibration_claims`.
- Pilot/dashboard readiness: `blocked`.
- Load modification readiness: `blocked`.
- `broad_30d`: 4 new prospective packets, at least 8 captured events, missed-event rate no greater than 0.75 before retesting.
- `severity_14d`: 4 new prospective packets, at least 8 captured events, missed-event rate no greater than 0.75 before retesting.
- Packet target mix for both retained channels: at least 2 monitoring-context packets, 1 missed-only error packet, and 1 outcome-context packet.

**Interpretation:** This reaches the next significant milestone: the project now has a concrete prospective evidence collection gate. The next operational work is collecting/adjudicating those prospective packets, not another calibration claim or product sprint.

## Previous Completed Step

**Exposure-load shadow bounded calibration stress-test sprint** - implemented and verified on 2026-05-17.

**What changed:** Added `exposure_load_shadow_bounded_calibration_stress_test.py`, `run_exposure_load_shadow_bounded_calibration_stress_test_sprint_experiment(...)`, and the `--exposure-load-shadow-bounded-calibration-stress-test-sprint` CLI mode. The sprint consumes the bounded protocol JSON and emits channel stress rows, scenario rows, gate rows, JSON, and a report.

**Live results (`exposure_load_shadow_bounded_calibration_stress_test_v1`):**
- Overall recommendation: `preserve_limited_calibration_finding_and_collect_more_prospective_evidence`.
- Production readiness: `not_ready_for_probability_or_pilot`.
- Calibration claim readiness: `not_ready_for_calibration_claims`.
- Pilot/dashboard readiness: `blocked`.
- Load modification readiness: `blocked`.
- `broad_30d`: 234 observed event rows, 37 captured, 197 missed, capture rate 0.158120, missed-event rate 0.841880, stress classification `high_miss_limited_calibration_signal`.
- `severity_14d`: 129 observed event rows, 18 captured, 111 missed, capture rate 0.139535, missed-event rate 0.860465, stress classification `high_miss_limited_calibration_signal`.
- Stress decision for both retained channels: `preserve_limited_monitoring_value_collect_more_evidence`.

**Interpretation:** The descriptive stress test preserves a limited monitoring finding, not a calibration finding suitable for claims. Do not move to probability-facing output, pilot/dashboard work, autonomous intervention, load modification recommendations, or calibration claims. The next allowed work is a prospective evidence collection gate for retained channels or a formal limited-finding closeout.

## Previous Completed Step

**Exposure-load shadow bounded calibration protocol sprint** - implemented and verified on 2026-05-15.

**What changed:** Added `exposure_load_shadow_bounded_calibration_protocol.py`, `run_exposure_load_shadow_bounded_calibration_protocol_sprint_experiment(...)`, and the `--exposure-load-shadow-bounded-calibration-protocol-sprint` CLI mode. The sprint consumes the error-control policy JSON and emits channel protocol rows, evidence-use rows, gate rows, policy JSON, and a report.

**Live results (`exposure_load_shadow_bounded_calibration_protocol_v1`):**
- Overall recommendation: `run_bounded_calibration_stress_test_without_claims`.
- Production readiness: `not_ready_for_probability_or_pilot`.
- Calibration claim readiness: `not_ready_for_calibration_claims`.
- Pilot/dashboard readiness: `blocked`.
- Load modification readiness: `blocked`.
- `broad_30d`: `eligible_for_bounded_stress_test_not_claims`; analysis scope `monitoring_context_error_controlled_only`; next analysis `bounded_calibration_stress_test_without_claims`.
- `severity_14d`: `eligible_for_bounded_stress_test_not_claims`; analysis scope `monitoring_context_error_controlled_only`; next analysis `bounded_calibration_stress_test_without_claims`.
- Required controls: miss-fraction control, monitoring-versus-prediction boundary, and packet-review control.

**Interpretation:** The next allowed research step is a descriptive shadow calibration stress test only. Monitoring context is not calibration evidence. Do not move to probability-facing output, pilot/dashboard work, autonomous intervention, load modification recommendations, or calibration claims.

## Previous Completed Step

**Exposure-load shadow error-control sprint** - implemented and verified on 2026-05-15.

**What changed:** Added `exposure_load_shadow_error_control.py`, `run_exposure_load_shadow_error_control_sprint_experiment(...)`, and the `--exposure-load-shadow-error-control-sprint` CLI mode. The sprint consumes the calibration-sensitivity JSON, writes channel decisions, refined evidence-dossier rows, required error controls, policy JSON, and a report, and keeps all outputs in research/shadow-monitoring posture.

**Live results (`exposure_load_shadow_error_control_v1`):**
- Overall recommendation: `continue_bounded_calibration_research_with_error_controls_not_claims`.
- Production readiness: `not_ready_for_probability_or_pilot`.
- Calibration claim readiness: `not_ready_for_calibration_claims`.
- Pilot/dashboard readiness: `blocked`.
- Load modification readiness: `blocked`.
- `broad_30d`: retained research candidate under error controls; monitoring status `supported_for_monitoring_context_only`; prediction/calibration status `not_established_high_miss_fraction`; next gate `continue_bounded_calibration_research_with_error_controls`.
- `severity_14d`: retained research candidate under error controls; monitoring status `supported_for_monitoring_context_only`; prediction/calibration status `not_established_high_miss_fraction`; next gate `continue_bounded_calibration_research_with_error_controls`.
- Required controls for both retained channels: miss-fraction control, monitoring-versus-prediction boundary, and packet review for empty or missed-only packets.

**Interpretation:** Both retained channels can continue only as bounded, error-controlled research candidates. Practitioner support counts as monitoring context, not prediction/calibration evidence. Do not move to probability-facing output, pilot/dashboard work, autonomous intervention, load modification recommendations, or calibration claims.

## Previous Completed Step

**Exposure-load shadow calibration sensitivity sprint** - implemented and verified on 2026-05-15.

**What changed:** Added `exposure_load_shadow_calibration_sensitivity.py`, `run_exposure_load_shadow_calibration_sensitivity_sprint_experiment(...)`, and the `--exposure-load-shadow-calibration-sensitivity-sprint` CLI mode. The sprint consumes practitioner-reviewed calibration readiness, the reviewed retained-channel collection CSV, and the retained event crosswalk, then writes channel sensitivity, evidence dossier, error-mode, JSON, and report artifacts.

**Live results (`exposure_load_shadow_calibration_sensitivity_v1`):**
- Overall recommendation: `continue_bounded_calibration_research_with_error_mode_controls`.
- Production readiness: `not_ready_for_probability_or_pilot`.
- Calibration claim readiness: `not_ready_for_calibration_claims`.
- Pilot/dashboard readiness: `blocked`.
- `broad_30d`: 4 practitioner rows, 2 useful/actionable rows, 37 captured event rows, 197 missed event rows, capture rate 0.158120, miss-rate gate `caution`.
- `severity_14d`: 4 practitioner rows, 3 useful/actionable rows, 18 captured event rows, 111 missed event rows, capture rate 0.139535, miss-rate gate `caution`.
- Error modes for both retained channels: `empty_outcome_packet`, `missed_only_packet`, `high_miss_fraction`, and `monitor_only_action_boundary`.

**Interpretation:** This is meaningful progress toward research utility but not product readiness. The retained channels have practitioner-supported monitoring value, yet the high miss fractions require explicit error-mode controls before any calibration research claim. Do not move to probability-facing output, pilot/dashboard work, autonomous intervention, or calibration claims.

## Previous Completed Step

**Exposure-load practitioner-reviewed shadow collection and calibration readiness bridge** - implemented and verified on 2026-05-15.

**What changed:** Added practitioner-adjudication accounting to the shadow collection summary and calibration-readiness review. The summary now distinguishes `complete_practitioner_adjudication` rows from `complete_csv_evidence_review` rows, and calibration readiness can clear only the independent-adjudication research gate while keeping all product/probability gates blocked. A separate practitioner-reviewed input was created at `outputs/experiments/exposure_load_shadow_collection_practitioner_review_v1/exposure_load_shadow_collection_practitioner_review.csv`; the original CSV-only prefill remains preserved.

**Live results (`exposure_load_shadow_collection_summary_practitioner_v1`):**
...
- Useful/source-trustworthy/actionable rows: 5.
- Practitioner-adjudicated rows: 8.
- CSV-only review rows: 0.
- Independent practitioner adjudication status: `satisfied`.

**Live results (`exposure_load_shadow_calibration_readiness_practitioner_v1`):**
- Overall recommendation: `advance_to_bounded_calibration_research_not_claims`.
- Production readiness: `not_ready_for_probability_or_pilot`.
- Calibration claim readiness: `not_ready_for_calibration_claims`.
- Calibration research status: `ready_for_bounded_calibration_research_not_claims`.
- Evidence basis: `independent_practitioner_adjudicated_shadow_collection`.
- Channel readiness: `broad_30d` and `severity_14d` are `calibration_research_candidate_practitioner_adjudicated`.
- Blocked gates: probability-facing outputs, pilot/dashboard readiness, and autonomous intervention.

**Interpretation:** The independent adjudication gate is now satisfied for retained-channel research review. The next allowed work is bounded calibration research sensitivity review. Do not move to probability-facing output, pilot/dashboard work, autonomous intervention, or calibration claims.

## Previous Completed Step

**Exposure-load shadow event-crosswalk sprint** - implemented and verified on 2026-05-14.

**What changed:** Added `exposure_load_shadow_event_crosswalk.py`, `run_exposure_load_shadow_event_crosswalk_sprint_experiment(...)`, and the `--exposure-load-shadow-event-crosswalk-sprint` CLI mode. The sprint recomputes retained-channel packet timelines from the exposure-load live inputs, filters to the current retained collection CSV when supplied, and writes a de-identified captured/missed injury-event crosswalk with injury context and nearest alert timing fields.

**Verification:** New TDD tests first failed because the module, runner, and CLI dispatch did not exist. A follow-up filter test first failed because the runner did not accept a retained collection filter. After implementation, focused event-crosswalk tests passed. The live command `risk-engine --measurements outputs/live_inputs/exposure_load_season_forward_validation_v1/canonical_measurements.csv --injuries outputs/live_inputs/exposure_load_season_forward_validation_v1/canonical_injuries.csv --exposure-participations outputs/exposure_inputs/exposure_cleaning_audit_v1/exposure_participations.csv --exposure-load-shadow-replay outputs/experiments/exposure_load_shadow_replay_v1/exposure_load_shadow_replay.json --exposure-load-shadow-collection outputs/experiments/exposure_load_shadow_collection_evidence_prefill_v1/exposure_load_shadow_collection_prefilled.csv --output-dir outputs --experiment-id exposure_load_shadow_event_crosswalk_v1 --graph-window-size 4 --model-variant l2 --exposure-load-shadow-event-crosswalk-sprint` completed.

**Live results (`exposure_load_shadow_event_crosswalk_v1`):**
- Packets: 8 retained-channel rows only.
- Event rows: 363.
- Captured events: 55.
- Missed events: 308.
- Packet count checks: all 8 retained packets match replay aggregate captured/missed counts.
- Output for reviewer use: `outputs/experiments/exposure_load_shadow_event_crosswalk_v1/exposure_load_shadow_event_crosswalk.csv`.

**Interpretation:** The reviewer no longer has to infer which injuries were included in captured counts from aggregate packet rows. Use the event crosswalk to adjudicate whether captured and missed de-identified events were meaningful. This does not support calibration claims, probability-facing output, pilot/dashboard readiness, or autonomous intervention.

## Previous Completed Step

**Exposure-load shadow calibration-readiness sprint** - implemented and verified on 2026-05-14.

**What changed:** Added `exposure_load_shadow_calibration_readiness.py`, `run_exposure_load_shadow_calibration_readiness_sprint_experiment(...)`, and the `--exposure-load-shadow-calibration-readiness-sprint` CLI mode. The sprint consumes a completed retained-channel collection summary JSON and writes channel readiness, evidence gap, JSON, and report artifacts.

**Verification:** New TDD tests first failed because the module, runner, and CLI dispatch did not exist. After implementation, focused calibration-readiness tests passed. The live command `risk-engine --exposure-load-shadow-calibration-readiness-sprint --exposure-load-shadow-collection-summary outputs/experiments/exposure_load_shadow_collection_summary_completed_v1/exposure_load_shadow_collection_summary.json --output-dir outputs --experiment-id exposure_load_shadow_calibration_readiness_v1` completed and wrote the readiness artifacts.

**Live results (`exposure_load_shadow_calibration_readiness_v1`):**
- Overall recommendation: `defer_calibration_claims_pending_independent_practitioner_adjudication`.
- Production readiness: `not_ready_for_probability_or_pilot`.
- Calibration claim readiness: `not_ready_for_calibration_claims`.
- Calibration research status: `research_candidate_pending_independent_practitioner_adjudication`.
- Evidence basis: `csv_only_artifact_review_requires_independent_practitioner_adjudication`.
- Channel readiness: `broad_30d` and `severity_14d` are `candidate_pending_independent_practitioner_adjudication`.
- Blocked gates: probability-facing outputs, pilot/dashboard readiness, and autonomous intervention.

**Interpretation:** One more bounded research sprint is complete. The next real evidence step is independent practitioner/source-context adjudication of the eight retained-channel rows. Do not proceed to calibration modeling, calibration claims, probability-facing output, pilot/dashboard work, or autonomous intervention before that adjudication is completed and summarized.
...
**Verification:** New TDD tests first failed because the prefill runner and CLI dispatch did not exist. After implementation and a schema fix for `review_packet_status` versus `replay_status`, focused tests passed. The live command `risk-engine --exposure-load-shadow-collection-evidence-prefill-sprint --exposure-load-shadow-review-packets outputs/experiments/exposure_load_shadow_replay_v1/exposure_load_shadow_review_packets.csv --output-dir outputs --experiment-id exposure_load_shadow_collection_evidence_prefill_v1` completed.

**Live results (`exposure_load_shadow_collection_evidence_prefill_v1`):**
- Overall recommendation: `review_prefilled_retained_channel_shadow_collection_rows`.
- Production readiness: `not_ready_for_probability_or_pilot`.
- Calibration readiness: `not_ready_for_calibration_claims`.
- Prefilled retained-channel rows: 8.
- Excluded rows: 4 `severity_7d` rows because the channel is not retained.
- Reviewer fields still required: `alert_usefulness`, `outcome_confirmed`, `source_context_ok`, `action_taken`, `reviewer_id`, and `review_date`.

**Interpretation:** The project no longer requires the user to manually recover replay-derived injury/event counts from scattered artifacts. The next user-facing file is `outputs/experiments/exposure_load_shadow_collection_evidence_prefill_v1/exposure_load_shadow_collection_prefilled.csv`; the user should fill only the reviewer judgment fields, then rerun the collection summary sprint.

## Previous Completed Step

**Exposure-load shadow collection packet workflow sprint** - implemented and verified on 2026-05-13.

**What changed:** Added `build_exposure_load_shadow_collection_packet_workflow(...)`, reviewer-instruction and packet-workflow report writers, `run_exposure_load_shadow_collection_packet_workflow_sprint_experiment(...)`, and the `--exposure-load-shadow-collection-packet-workflow-sprint` CLI mode. The sprint consumes the retained-channel collection CSV and writes reviewer packet manifests, per-packet checklists, audit-trail seed rows, reviewer instructions, and one de-identified packet markdown file per pending retained-channel packet.

**Verification:** New TDD tests first failed because the packet-workflow module function, runner, and CLI dispatch did not exist. After implementation, focused packet-workflow tests passed. The live command `risk-engine --exposure-load-shadow-collection-packet-workflow-sprint --exposure-load-shadow-collection outputs/experiments/exposure_load_shadow_collection_template_v1/exposure_load_shadow_collection_template.csv --output-dir outputs --experiment-id exposure_load_shadow_collection_packet_workflow_v1` completed.

**Live results (`exposure_load_shadow_collection_packet_workflow_v1`):**
- Overall recommendation: `prepare_retained_channel_reviewer_packets`.
- Production readiness: `not_ready_for_probability_or_pilot`.
- Calibration readiness: `not_ready_for_calibration_claims`.
- Reviewer packet count: 8.
- Checklist rows: 56.
