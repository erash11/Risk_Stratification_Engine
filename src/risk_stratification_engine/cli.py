...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.exposure_load_shadow_collection_summary_sprint:
        if args.exposure_load_shadow_collection is None:
            parser.error(
                "--exposure-load-shadow-collection-summary-sprint requires "
    ...
    if args.exposure_load_shadow_calibration_sensitivity_sprint:
        required_paths = {
            "--exposure-load-shadow-calibration-readiness": (
                args.exposure_load_shadow_calibration_readiness
            ),
            "--exposure-load-shadow-collection": args.exposure_load_shadow_collection,
            "--exposure-load-shadow-event-crosswalk": (
                args.exposure_load_shadow_event_crosswalk
            ),
        }
        missing = [flag for flag, path in required_paths.items() if path is None]
        if missing:
            parser.error(
                "--exposure-load-shadow-calibration-sensitivity-sprint requires "
                + ", ".join(missing)
            )
        experiment_dir = (
            run_exposure_load_shadow_calibration_sensitivity_sprint_experiment(
        ...
        print(
            "Exposure load shadow calibration sensitivity artifacts written to "
            f"{experiment_dir}"
        )
        return 0

    if args.exposure_load_shadow_error_control_sprint:
        if args.exposure_load_shadow_calibration_sensitivity is None:
            parser.error(
                "--exposure-load-shadow-error-control-sprint requires "
                "--exposure-load-shadow-calibration-sensitivity"
            )
        experiment_dir = run_exposure_load_shadow_error_control_sprint_experiment(
            exposure_load_shadow_calibration_sensitivity_path=(
                args.exposure_load_shadow_calibration_sensitivity
            ),
            output_dir=args.output_dir,
            experiment_id=args.experiment_id,
        )
        print(
            "Exposure load shadow error-control artifacts written to "
            f"{experiment_dir}"
        )
        return 0

    if args.exposure_load_shadow_bounded_calibration_protocol_sprint:
        if args.exposure_load_shadow_error_control_policy is None:
            parser.error(
                "--exposure-load-shadow-bounded-calibration-protocol-sprint "
                "requires --exposure-load-shadow-error-control-policy"
            )
        experiment_dir = (
            run_exposure_load_shadow_bounded_calibration_protocol_sprint_experiment(
                exposure_load_shadow_error_control_policy_path=(
                    args.exposure_load_shadow_error_control_policy
                ),
                output_dir=args.output_dir,
                experiment_id=args.experiment_id,
            )
    ...
    if args.exposure_load_shadow_prospective_collection_completion_sprint:
